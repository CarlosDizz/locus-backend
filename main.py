...
def recent_turns_to_text(turns: list[dict], limit: int = 12) -> str:
    ...
    for turn in selected:
        lines.append(f"{turn.get('role', 'user').upper()}: {turn.get('text', '')}")
    return "\n".join(lines)
...
def build_verified_poi_context(poi_name: str, answer_goal: str = "") -> str:
    ...
    if not raw_text:
        ...
        return ""
    verified_context = build_verified_context_from_text(poi_name, raw_text, answer_goal)
    if verified_context:
        poi_context_cache[cache_key] = verified_context
...
async def home_chat(req: ChatRequest):
    ...
    if req.action == "setup_profile":
        ...
        if active_poi and not room["verified_context"]:
            room["verified_context"] = await asyncio.to_thread(build_verified_poi_context, active_poi, "Dar contexto inicial breve y fiable del POI activo.")
        prompt = prompts.CHAT_SETUP_PROMPT.format(user_context=req.context or "(sin contexto)", active_poi=active_poi or "(sin POI activo)", nearby_pois=nearby_pois)
        reply = await asyncio.to_thread(generate_text, CONTENT_MODEL, prompt)
        append_turn(history, "assistant", reply)
        pois_block = f"\n<POIS>{json.dumps(real_pois, ensure_ascii=False)}</POIS>" if real_pois else ""
    ...
    pois_block = f"\n<POIS>{json.dumps(real_pois, ensure_ascii=False)}</POIS>" if real_pois else ""
    analysis = await asyncio.to_thread(analyze_turn, active_poi, base_context, room["verified_context"], history, user_turn)
    retrieval_target = analysis.get("retrieval_query", "") or active_poi
    if analysis.get("needs_retrieval") and retrieval_target:
        verified_context = await asyncio.to_thread(build_verified_context_from_query, retrieval_target, active_poi, analysis.get("answer_goal", req.text or ""))
        if verified_context:
            room["verified_context"] = verified_context
    reply = await asyncio.to_thread(answer_user, active_poi, base_context, room["verified_context"], history, user_turn, analysis.get("answer_goal", ""))
    if not reply:
...
async def get_token(req: TokenRequest):
    ...
    if active_poi:
        verified_context = await asyncio.to_thread(build_verified_poi_context, active_poi, "Preparar contexto inicial fiable para la visita.")
        if verified_context:
...
async def entrypoint(ctx: JobContext):
    ...
    if active_poi:
        initial_verified_context = await asyncio.to_thread(build_verified_poi_context, active_poi, "Preparar contexto inicial fiable para la visita.")
    ...
    async def publish_room_event(payload: dict):
        try:
            await ctx.room.local_participant.publish_data(json.dumps(payload, ensure_ascii=False).encode("utf-8"), reliable=True)
        except Exception as e:
            log(f"publish_room_event error: {e}")

    def cleanup_audio_buffers():
        now = asyncio.get_event_loop().time()
        expired = [audio_id for audio_id, item in state["audio_buffers"].items() if now - item["created_at"] > 180]
        ...
        def transcribe():
            resp = gemini_client.models.generate_content(
                ...
                contents=[
                    ...
                    types.Part.from_text(text="Transcribe con precisión el audio en español. Devuelve solo la transcripción sin comentarios.")
                ]
            )
            return clean_text(resp.text or "")
        return await asyncio.wait_for(asyncio.to_thread(transcribe), timeout=20)

    async def speak_planned_answer(phase: str, planned_answer: str, active_focus: str, user_turn: str):
        planned_answer = clean_text(planned_answer)
        if not planned_answer:
            return
        async with speech_lock:
            log(f"speak_text START phase={phase} text={planned_answer[:180]}")
            instructions = (
                "Responde AHORA al usuario por voz.\n"
                "Reglas obligatorias:\n"
                "- Sigue fielmente las ideas clave de la RESPUESTA PLANIFICADA.\n"
                "- No cambies de tema.\n"
                "- No añadas lugares distintos.\n"
                "- No expliques procesos internos.\n"
                "- Habla en español natural y claro.\n"
                "- Si la respuesta planificada ya responde, no repreguntes.\n\n"
                f"POI EN FOCO:\n{active_focus or '(sin POI activo)'}\n\n"
                f"ÚLTIMO TURNO DEL USUARIO:\n{user_turn}\n\n"
                f"RESPUESTA PLANIFICADA:\n{planned_answer}"
            )
            await session.generate_reply(instructions=instructions)
            log(f"speak_text END phase={phase}")

    async def store_agent_shadow(text: str, segment_id: str = ""):
        text = clean_text(text)
        if not text:
            return
        dedupe = clean_text(segment_id or text[:120])
        if dedupe and dedupe in state["seen_agent_segments"]:
            return
        if dedupe:
            state["seen_agent_segments"].add(dedupe)
        append_turn(state["history"], "assistant", text)
        log(f"AGENT SHADOW stored text={text[:160]}")

    async def process_user_turn(user_text: str = "", image_description: str = "", source: str = "text", sender_label: str = "Tú", sender_role: str = ""):
        async with turn_lock:
            clean_user_text = clean_text(user_text)
            sub_poi = infer_sub_poi(clean_user_text)
            if sub_poi:
                state["current_sub_poi"] = sub_poi
            user_turn = build_user_turn_text(user_text=clean_user_text, image_description=image_description)
            ...
            log(f"TURN START source={source} focus={effective_focus}")
            log(f"TURN user_turn={user_turn[:280]}")

            analysis = await asyncio.wait_for(asyncio.to_thread(analyze_turn, effective_focus, state["base_context"], state["verified_context"], state["history"], user_turn), timeout=12)
            log(f"analyze_turn END analysis={analysis}")
            retrieval_target = clean_text(analysis.get("retrieval_query", "")) or state["current_sub_poi"] or state["active_poi"]
            if analysis.get("needs_retrieval") and retrieval_target:
                log(f"retrieval START target={retrieval_target}")
                verified_context = await asyncio.wait_for(
                    asyncio.to_thread(build_verified_context_from_query, retrieval_target, state["active_poi"], analysis.get("answer_goal", clean_user_text or image_description)),
                    timeout=15,
                )
                if verified_context:
            ...
            planned_answer = await asyncio.wait_for(
                asyncio.to_thread(answer_user, effective_focus, state["base_context"], state["verified_context"], state["history"], user_turn, analysis.get("answer_goal", "")),
                timeout=18,
            )
            planned_answer = clean_text(planned_answer)
            if not planned_answer:
                planned_answer = "No tengo ese dato confirmado con suficiente seguridad, pero puedo seguir ayudándote con este lugar."
            log(f"planned_answer={planned_answer[:220]}")
            await speak_planned_answer("answer", planned_answer, effective_focus, user_turn)
            log("TURN END OK")

    async def handle_voice_input(payload: dict):
        try:
            ...
            sender_identity = payload.get("participant_identity", "")
            if not audio_id or chunk_index < 0 or total_chunks <= 0 or not chunk_data:
                log("voice_input invalid payload")
                return
            ...
            for i in range(total_chunks):
                ...
                ordered.append(part)
            del state["audio_buffers"][audio_id]
            audio_bytes = base64.b64decode("".join(ordered))
            transcription = await transcribe_audio_bytes(audio_bytes, mime_type)
            log(f"voice_input transcription={transcription}")
            if not transcription:
                return
            await publish_room_event({
                "action": "participant_transcription_echo",
                "text": transcription,
                "sender_role": sender_role,
                "sender_identity": sender_identity,
            })
            await process_user_turn(user_text=transcription, image_description="", source="voice", sender_role=sender_role, sender_label=sender_identity or "Participante")
        except Exception as e:
            log(f"handle_voice_input error: {e}")
        ...
        try:
            ...
            if action == "text_chat":
                async def handle_text_chat():
                    text = clean_text(payload.get("data", ""))
                    if not text:
                        return
                    log(f"TEXT_CHAT recibido text={text}")
                    await process_user_turn(user_text=text, image_description="", source="text", sender_role=payload.get("role", ""))
                asyncio.create_task(handle_text_chat())
            elif action == "voice_input_chunk":
                asyncio.create_task(handle_voice_input(payload))
                ...
                async def handle_image():
                    try:
                        ...
                        def describe_image():
                            ...
                            return clean_text(resp.text or "")
                        image_description = await asyncio.wait_for(asyncio.to_thread(describe_image), timeout=20)
                        log(f"image_context description={image_description[:220]}")
                        await process_user_turn(user_text=text_hint, image_description=image_description, source="image", sender_role=payload.get("role", ""))
                    except Exception as e:
                        log(f"handle_image error: {e}")
                asyncio.create_task(handle_image())
            elif action == "agent_shadow":
                asyncio.create_task(store_agent_shadow(payload.get("text", ""), payload.get("segment_id", "")))
                ...
                async def handle_update():
                    ...
                    if new_poi:
                        ...
                        state["verified_context"] = await asyncio.wait_for(
                            asyncio.to_thread(build_verified_poi_context, new_poi, "Actualizar contexto factual del nuevo POI activo."),
                            timeout=12,
                        )
                        state["history"] = []
    ...
    try:
        poi_name = state["active_poi"] or "este lugar"
        welcome_text = f"Hola. Soy Locus, tu guía por voz. Estamos en {poi_name}. Puedes preguntarme por voz, texto o foto y te responderé siempre hablando."
        await speak_planned_answer("welcome", welcome_text, poi_name, "Bienvenida inicial")
    except Exception as e:
