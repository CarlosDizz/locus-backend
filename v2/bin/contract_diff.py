...
Run it right before the cutover, with both stacks up:
    ...
    # V1 (its .env points MYSQL_PUBLIC_URL at a dead Railway host, and its config
    # prefers that over DATABASE_URL, so the override pins the local database)
    cd locus-backend-1
    docker compose -f docker-compose.yml -f v2/bin/v1-local-db.yml up -d
    docker compose -f docker-compose.yml -f v2/bin/v1-local-db.yml exec api alembic upgrade head

    # V2
    cd v2 && LOCUS_API_HOST_PORT=8200 docker compose up -d

    python3 v2/bin/contract_diff.py [token_v1] [token_v2]
